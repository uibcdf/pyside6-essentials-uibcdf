...
cmake -S "${SRC_DIR}" -B "${SRC_DIR}/build-conda" -G Ninja     -DCMAKE_INSTALL_PREFIX="${PREFIX}"     -DCMAKE_PREFIX_PATH="${PREFIX}"     -DPython_EXECUTABLE="${PYTHON}"     -DPYTHON_SITE_PACKAGES="${SP_DIR}"     -DBUILD_TESTS=OFF     -DDISABLE_PYI=ON
