...
cmake -S "${SRC_DIR}" -B "${SRC_DIR}/build-conda" -G Ninja     -DCMAKE_INSTALL_PREFIX="${PREFIX}"     -DCMAKE_PREFIX_PATH="${PREFIX}"     -DCMAKE_FIND_USE_PACKAGE_REGISTRY=OFF     -DCMAKE_FIND_ROOT_PATH_MODE_PACKAGE=ONLY     -DPython_EXECUTABLE="${PYTHON}"     -DPYTHON_SITE_PACKAGES="${SP_DIR}"     -DBUILD_TESTS=OFF     -DDISABLE_PYI=ON     -DMODULES='Core;Gui;Widgets;PrintSupport;Sql;Network;Test;Concurrent;DBus;Designer;Xml;Help;OpenGL;OpenGLWidgets;Qml;Quick;QuickControls2;QuickTest;QuickWidgets;Svg;SvgWidgets;UiTools;WebChannel;WebSockets'
